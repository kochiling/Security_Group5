...
if (isset($_POST['otp']) && isset($_POST['email'])) {
    ...
    if (mysqli_stmt_execute($stmt2) && mysqli_num_rows($result) > 0) {
        ...
        $log->error('Unable to update password', ['email' => $email]);
    }
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) && domain_exists($email)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            try {
                ...
                $_SESSION['otp'] = $OTP . "";
                $log->info('Password request OTP sent to email', ['email' => $email]);
            } catch (Exception $e) {
                $response['status'] = 'ERROR';
                $response['message'] = 'Something went wrong!';
                $log->error('Error sending password reset request to email', ['email' => $email]);
            }
        } else {
            $response['status'] = 'ERROR';
            $response['message'] = 'Email not found!';
            $log->error('Email not found in system', ['email' => $email]);
        }
        mysqli_stmt_close($stmt);
    } else {
        $response['status'] = 'ERROR';
        $response['message'] = 'Invalid email!';
        $log->error('Invalid email for password reset', ['email' => $email]);
    }
} else {
    $response['status'] = 'ERROR';
    $response['message'] = 'Somehing went wrong!';
    $log->error('Invalid request for password reset');
}
